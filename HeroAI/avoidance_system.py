...
UPDATE_FREQUENCY = 25           # Milliseconds between updates
...
class AvoidanceSystem:
    ...
    def update(self, target_id):
        ...
        # Skip updates if we're already moving and path is valid
        if self.is_active and self.path and self.current_path_index < len(self.path):
            # Only check if target moved significantly
            if self.avoidance_cooldown.HasElapsed(UPDATE_FREQUENCY * 2):
                ...
                if Utils.Distance(target_pos, self.last_target_position) > TARGET_MOVED_THRESHOLD:
                    self.find_path_around_obstacles(Player.GetXY(), self.target_id)
                else:
                    return self.follow_path()
            else:
                return self.follow_path()
            
        # Normal update logic if not actively following path
